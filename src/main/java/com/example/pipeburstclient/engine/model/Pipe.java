...
public class Pipe {
    ...
    public Pipe(double length, double diameter, int horizontalQuantity, int verticalQuantity, List<Point> startVerticalPoints) {
        ...
        this.points = calculationNormalPipePoints(startVerticalPoints);
    }
    ...
    public List<List<Point>> calculationNormalPipePoints(List<Point> startVerticalPoints) {
        initializePoints();
    ...
    public void calculationBurstPipePoints(double length_b, double burstCoefficient){
        burstCoefficient *= 10;
        Calculation calc = new Calculation();
        double density0 = points.get(0).get(0).density;
        double density1 = density0 * burstCoefficient;
        double density_step = 0;
        double velocity0 = points.get(verticalQuantity / 2).get(0).velocity;
        double velocity1 = velocity0 * burstCoefficient;
            ...
            for (int j = 1; j < horizontalQuantity; j++) {
                ...
                if ((x < -1 * Math.sqrt(1 / diameter) * Math.pow(y - diameter, 2) + length_b))
                    ...
                    if (points.get(i).get(j - 1).density == density0) {
                        double point_c = (length_b / deltaLength - j * 0.9);
                    ...
                    points.get(i).get(j).velocity = points.get(i).get(j - 1).velocity - velocity_step;
